...
function resolveStackFolder(language) {
...
export class ReverseWorkflow {
    ...
    async execute(options) {
        ...
        const stackFolder = resolveStackFolder(config.stack.language);
        ...
        const constitutionTemplate = await this.templateLoader.load(stackFolder, 'reverse/constitution');
        await fs.writeFile(path.join(reversePath, 'constitution.md'), this.templateRenderer.render(constitutionTemplate, variables));
        const metaTemplate = await this.templateLoader.load(stackFolder, 'reverse/meta');
        await fs.writeFile(path.join(reversePath, 'meta.md'), this.templateRenderer.render(metaTemplate, variables));
        return {
            success: true,
            reversePath,
            nextSteps: [
...
//# sourceMappingURL=ReverseWorkflow.js.map