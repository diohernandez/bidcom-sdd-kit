...
export class TestingDetector {
  async detect(projectPath: string, language: Language): Promise<DetectedStackTesting> {
    if (language === 'typescript') return this.detectTypeScriptTesting(projectPath)
    if (language === 'python') return this.detectPythonTesting(projectPath)
    return {}
  }

  private async detectTypeScriptTesting(projectPath: string): Promise<DetectedStackTesting> {
    ...
    if (deps.playwright) e2e.push(`Playwright ${stripSemverRange(deps.playwright)}`)

    const visual: string[] = []
    if (deps.storybook) visual.push(`Storybook ${stripSemverRange(deps.storybook)}`)
  ...
  private async detectPythonTesting(projectPath: string): Promise<DetectedStackTesting> {
