...
export const DEFAULT_TEMPLATES_ROOT = path.resolve(
  moduleDir,
  "../../../templates",
);
...
export class TemplateLoader {
  constructor(
    private readonly templatesRoot: string = DEFAULT_TEMPLATES_ROOT,
  ) {}
  ...
  async loadForStack(stack: { language: Language }): Promise<TemplateSet> {
    ...
    const entries = await Promise.all(
      (
        Object.entries(DEV_WORKFLOW_TEMPLATE_FILES) as [
          TemplateSetKey,
          string,
        ][]
      ).map(
        async ([key, fileName]) =>
          [key, await this.load(stackFolder, fileName)] as const,
      ),
    );
    return Object.fromEntries(entries) as TemplateSet;
  }

  resolveStackFolder(language: Language): string {
