...
function validateFunctionalSpec(content: string): ValidateCheck[] {
  return [
...
function validateTechnicalSpec(content: string): ValidateCheck[] {
  return [
    ...
    if (phase === "tecnico") {
      ...
      if (!(await fileExists(specPath))) {
        return {
          success: false,
          phase,
          error: `No existe la especificación técnica (${specPath})`,
        };
      }
      const specContent = await fs.readFile(specPath, "utf-8");
